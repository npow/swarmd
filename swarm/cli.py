...
async def launch(mission_yaml: str, workspace: str | None) -> None:
    ...
    from swarm.mcp.server import _launch_mission
...
def main() -> None:  # pragma: no cover — click handles the real entry
    """Console-script entry point — used by ``swarm = "swarm.cli:cli"``.
...
# Stable public surface so ``from swarm.cli import cli`` works without
